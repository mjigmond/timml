...
class CircleInhom(Element):
    ...
    def setCoefs(self):
        self.xc = self.aqin.xc; self.yc = self.aqin.yc; self.R = self.aqin.R; self.Rsq = self.aqin.Rsq
    ...
    def potentialInfluence(self,aq,x,y):
        ...
        mat = zeros( (self.order+1,aq.Naquifers), 'd' )  # matrix to store values
     # Inside Cylinder
        if aq == self.aqin:
        ...
        if aq == self.aqin:  # Inside cylinder
            ...
            if aq.type == aq.conf:  # Confined aquifer
                for p in range(self.order+1):
                    if p == 0:
                        ...
                        mat[p,0] = - float(p) * r**(p-1)  # first value in row is pr**(p-1)
                    mat[p,1:] = - ( scipy.special.iv(p-1,rolab) + scipy.special.iv(p+1,rolab) ) / (2 * aq.lab)
            elif aq.type == aq.semi:
                for p in range(self.order+1):
                    mat[p,:] = - ( scipy.special.iv(p-1,rolab) + scipy.special.iv(p+1,rolab) ) / (2 * aq.lab)
            mat = mat * self.matRin
            ...
            if aq.type == aq.conf:
                for p in range(self.order+1):
                    ...
                    mat[p,1:] = ( scipy.special.kn(p-1,rolab) + scipy.special.kn(p+1,rolab) ) / (2 * aq.lab) 
                mat[0,0] = - 1.0 / (2.0*pi*r)  # log
            elif aq.type == aq.semi:
                for p in range(self.order+1):
                    mat[p,:] = ( scipy.special.kn(p-1,rolab) + scipy.special.kn(p+1,rolab) ) / (2 * aq.lab) 
            mat = mat * self.matRout
        else:  # Somewhere else
            if self.aqout.type == self.aqout.conf and aq.type == aq.conf:
    ...
    # Tangential part
        ...
        if aq == self.aqin:  # Inside cylinder
            rolab = r/aq.lab  # vector with all r/lab values
            if aq.type == aq.conf:
               for p in range(1,self.order+1):  # all values for p=0 are zero (no dependence on theta)
                    mat[p,0] = float(p) * r**(p-1)  # first value in row is pr**(p-1)
            ...
            if aq.type == aq.conf:
                for p in range(1,self.order+1):  # all values for p=0 are zero (no dependence on theta)
                    mat[p,0] = float(p)/r**(p+1)  # first value in row is 1/r**p
    ...
    def nearElement(self,pyLayer,xyz1,xyz2,step,idir):
        ...
        if ( r1sq < self.Rsq and r2sq > self.Rsq  ) or ( r1sq > self.Rsq and r2sq < self.Rsq  ):
            ...
            c = self.xc**2 + self.yc**2 + x1**2 + y1**2 - 2.0 * (self.xc*x1 +self.yc*y1) - self.Rsq
