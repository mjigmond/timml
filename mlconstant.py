...
class Constant(Element):
    ...
    - layer: list of layer number in which reference point is placed; must be length 1
    Attributes computed:
    - pylayer: Python layer number in which reference point is placed (one less than layer)
    - pot: potential equivalent of specified head
    All attributes from Element
    '''
    def __init__(self,modelParent,xr,yr,head,layer=[1],label=None):
    ...
    def __repr__(self):
	return 'Constant xr,yr,head,C,layer: ' + str((self.xr,self.yr,self.head,self.parameters[0,0],self.pylayer+1))
    def setCoefs(self):
        self.pylayer = self.layer[0] - 1;
    ...
    def takeParameters(self,xsol,icount):
        ...
        return icount+1
    def check(self):
        print 'Constant at '+str((self.xr,self.yr))+' in layer '+str(self.layer[0])
        print 'Specified head: '+str(self.head)+' Computed head: '+str(self.modelParent.head(self.layer[0],self.xr,self.yr))+\
