'''
mlcircareasink.py contains the CircAreaSink class
...
class CircAreaSink(Element):
    def __init__(self,modelParent,xp,yp,Rp,infil,layer=[1],label=None):
	Element.__init__(self,modelParent)
        self.xp = float(xp); self.yp = float(yp); self.Rp = float(Rp); self.infil = float(infil)
        self.layer=layer; self.pylayer = layer[0]-1;
        assert len(self.layer)==1, "Input error: CircAreaSink can only be defined in one layer"
        self.label = label
        self.type = 'circareasink'
        self.setCoefs()
        self.modelParent.addElement(self)
    def __repr__(self):
	return 'CircAreaSink xp,yp,Rp,infil,layer: ' + str((self.xp,self.yp,self.Rp,self.infil,self.pylayer+1))
    def setCoefs(self):
        self.Rpsq = self.Rp**2
    ...
    def layout(self):
        ...
        return rv
    def check(self):
        print 'CircAreaSink at '+str((self.xp,self.yp))+' Layer: '+str(self.layer[0])+\
